...
def run (lib_idx, rept, nom, foo, tipo, m, n, k, alpha, A, lda, B, ldb, beta, F, ldc, blk):
    secs = time()
    ...
    print(f"{current_foo};{nom};{m}x{n}x{k};{TIEMPO:1.5E};{LA.norm(D-F, 'fro'):1.5E}")
...
for i in range(0,len(talla)):
  ...
  run (lib_idx, rept, "IccO0", IccO0, tipo, m, n, k, alpha, A, m, B, k, beta, F, m, blk)

  F = np.asarray(C, order='F')
  run (lib_idx, rept, "IccO3", IccO3, tipo, m, n, k, alpha, A, m, B, k, beta, F, m, blk)
